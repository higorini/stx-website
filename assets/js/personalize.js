...
let path = "./assets/images/personalize/";
...
function alternateGroups() {
  ...
  groups.forEach((group, index) => {
    const images = group.querySelectorAll(".vitrine__img:not(.ignore)");
    const waitTime = index * 1000 + images.length * 200;
  ...
  setTimeout(
    ...
    totalWaitTime + 1000 + Math.floor(Math.random() * (2000 - 1000 + 1)) + 1000
  );
}
...
function contactOnWhatsapp() {
  window.open("https://wa.me/+351914636204?text=Olá! Tenho interesse nas camisetas personalizadas, poderia me ajudar?", "_blank")
}